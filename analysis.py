...
def count_tokens(text):
    """
    Conta il numero di token in un testo usando spaCy.
    Più efficiente dei semplici split().
    """
    doc = nlp(text)
    return len(doc)
...
def analyze_token_metrics(book_name, output_dir):
    ...
    Genera un file CSV e dati per grafici.
    """
    ...
    for chapter_file in chapter_files:
        ...
        # Recupera il testo originale del capitolo
        chapter_text = ""
        with open(file_path, 'r', encoding='utf-8') as cf:
            reader = csv.reader(cf)
            for row in reader:
                if len(row) >= 2 and row[0] == "Original Text":
                    chapter_text = row[1]
                    break
        
        # Se non troviamo il testo originale, dobbiamo modificare analyze_chapter
        # per salvarlo nel file di analisi
        if not chapter_text:
            continue
...
def main():
    try:
        ...
        token_metrics_file = analyze_token_metrics(book_name, output_dir)
        context_metrics_file = analyze_context_consistency(book_name, output_dir)
        
        # Valutazione del modello BERT
        bert_evaluation = evaluate_bert_location_classifier()
