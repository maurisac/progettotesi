...
def find_chapters(text):
    ...
    # Prima identifica gli inizi di capitolo che hanno il formato "Capitolo X" o "Chapter X"
    for match in re.finditer(r'\b(?:Capitolo|Chapter)\s+(\d+)\b', text, re.IGNORECASE):
        ...
        print(f"      - Trovato: Capitolo {chapter_num} in posizione {start_byte}")
        print("      - Testo: '{}'...".format(text[start_byte:start_byte+50].replace('\n', '\\n')))
    
    # Poi cerca anche i numeri seguiti da un accapo (nuovo formato)
    # Il pattern cerca un numero su una riga, seguito da un accapo, seguito da una lettera maiuscola
    for match in re.finditer(r'(?:^|\n)(\d+)\s*\n\s*([A-Z])', text):
        chapter_num = int(match.group(1))
        start_byte = match.start() + 1  # +1 per escludere il newline precedente se presente
        
        # Aggiungi solo se non è già stato trovato con il formato precedente
        if chapter_num not in chapter_starts:
            chapter_starts[chapter_num] = start_byte
            print(f"      - Trovato: Capitolo {chapter_num} (formato numerico) in posizione {start_byte}")
            print("      - Testo: '{}'...".format(text[start_byte:start_byte+50].replace('\n', '\\n')))
...
def analyze_chapter(book_name, chapter_num, chapter_text, output_dir):
    ...
    try:
        ...
        with open(file_name, "a", newline='', encoding="utf-8") as f:
            ...
            # Salva un estratto del testo originale per analisi successive
            # (per non appesantire troppo il file)
            text_extract = chapter_text[:min(5000, len(chapter_text))]
            writer.writerow(["Original Text Extract", text_extract])
            writer.writerow(["Token Count", count_tokens(chapter_text)])
...
def count_tokens(text):
    """Conta il numero di token in un testo usando spaCy. Utile per valutare la complessità."""
    doc = nlp(text)
    return len(doc)
